...
inline void preprocess_marginal_field(int time_index, int start_xid, int start_yid, int end_xid, int end_yid, vector<float> coef_list_large, vector<float> coef_list_small)
{
    int x_idx, y_idx;
    for (x_idx = start_xid; x_idx < end_xid; x_idx++)
    {
        if (x_idx == start_xid)
        {
            field[time_index - 1][start_yid][start_xid] -= field[time_index - 1][start_yid][start_xid - 1] * coef_list_small[1];
            field[time_index - 1][start_yid][start_xid] -= field[time_index - 1][start_yid - 1][start_xid] * coef_list_small[0];
            field[time_index - 1][end_yid - 1][start_xid] -= field[time_index - 1][end_yid - 1][start_xid - 1] * coef_list_small[1];
            field[time_index - 1][end_yid - 1][start_xid] -= field[time_index - 1][end_yid][start_xid] * coef_list_small[4];
        }
        else if (x_idx == end_xid - 1)
        {
            field[time_index - 1][start_yid][x_idx] -= field[time_index - 1][start_yid][x_idx + 1] * coef_list_large[3];
            field[time_index - 1][start_yid][x_idx] -= field[time_index - 1][start_yid - 1][x_idx] * coef_list_large[0];
            field[time_index - 1][end_yid - 1][x_idx] -= field[time_index - 1][end_yid - 1][x_idx + 1] * coef_list_large[3];
            field[time_index - 1][end_yid - 1][x_idx] -= field[time_index - 1][end_yid][x_idx] * coef_list_large[4];
        }
        else
        {
            field[time_index - 1][start_yid][x_idx] -= field[time_index - 1][start_yid - 1][x_idx] * coef_list_small[0];
            field[time_index - 1][end_yid - 1][x_idx] -= field[time_index - 1][end_yid][x_idx] * coef_list_small[4];
        }
    }
    for (y_idx = start_yid + 1; y_idx < end_yid - 1; y_idx++)
    {
        field[time_index - 1][y_idx][start_xid] -= field[time_index - 1][y_idx][start_xid - 1] * coef_list_small[1];
        field[time_index - 1][y_idx][end_xid - 1] -= field[time_index - 1][y_idx][end_xid] * coef_list_large[3];
    }
}

inline void postprocess_marginal_field(int time_index, int start_xid, int start_yid, int end_xid, int end_yid, vector<float> coef_list_large, vector<float> coef_list_small)
{
    int x_idx, y_idx;
    for (x_idx = start_xid; x_idx < end_xid; x_idx++)
    {
...
void explicit_solve_2d(int now_step, float calc_time_step, map<string, float> delta, map<string, pair<float, float>> domain_col, float coef = 1., pair<float, float> neighb_coef={1., 1.})
{
    int t, i, j;
...
void implicit_solve_2d(int now_step, float calc_time_step, map<string, float> delta, map<string, pair<float, float>> domain_col, float coef = 1.)
{
    ...
    for (i = now_step; i < now_step + calc_step; i++)
    {
        ...
        preprocess_marginal_field(i, start_xid, start_yid, end_xid, end_yid, coef_list, coef_list);
        ...
        postprocess_marginal_field(i, start_xid, start_yid, end_xid, end_yid, coef_list, coef_list);
...
void sparse_symmetric_implicit_solve_2d_xy(int now_step, float calc_time_step, map<string, float> delta, map<string, pair<float, float>> domain_col, float coef = 1., pair<float, float> neighb_coef={1., 1.})
{
    ...
    for (i = now_step; i < now_step + calc_step; i++)
    {
        ...
        preprocess_marginal_field(i, start_xid, start_yid, end_xid, end_yid, coef_list_large, coef_list_small);
        ...
        postprocess_marginal_field(i, start_xid, start_yid, end_xid, end_yid, coef_list_large, coef_list_small);
        // cout << " 4 n rows:" << coef_n_rows <<" n cols:" << coef_n_cols<<  endl;
...
void sparse_symmetric_implicit_solve_2d_yx(int now_step, float calc_time_step, map<string, float> delta, map<string, pair<float, float>> domain_col, float coef = 1., pair<float, float> neighb_coef={1., 1.})
{
    printf("Not Recommend!");
    exit(1);
    int t, i, j, k, index, x_idx, y_idx, coef_idx, field_xid, field_yid, coef_k, dense_index, max_gauss_coef_index, add_index, coef_row_base;
    ...
    // cout << "r_x:" << ratio_x << " r_y:" << ratio_y << endl;
    int f2c_idx = 0;
    int x_idx_num, y_idx_num, coef_n_rows, coef_n_cols;
    x_idx_num = end_xid - start_xid;
    y_idx_num = end_yid - start_yid;
...
void explicit_solve_2d_whole(int now_step, float calc_time_step, map<string, float> delta, vector<map<string, pair<float, float>>> domain_col)
{
    int t, i, j, k;
    // int calc_step = int(calc_time_step / delta["t"]);
    int calc_step = 1;
    int start_xid, end_xid, start_yid, end_yid;
    int n_cols1 = field[0].size();
    float x_diff, ratio_x, ratio_y, y_diff, ratio_tx, ratio_ty, coef, coef_2, ratio_neighb;


    start_yid = max((int)domain_col[0]["y"].first, 1);
    end_yid = domain_col[domain_col.size()-1]["y"].second;
    // cout << "ok exp1" << endl;
    // cout << "---------- exp -----------" << endl;
    // cout << "start y:" << domain_col["y"] << endl;
    // cout << "start x:" << domain_col["x"] << endl;
    ...
    ratio_ty = delta["t"] / (delta["y"] * delta["y"]);
    // cout << "exp r_x:" << ratio_x << " r_y:" << ratio_y << endl;

    float ratio = coef * delta["t"] * (1/(delta["x"]*delta["x"]) + 1/(delta["y"]*delta["y"]));
    if(ratio >= 1./2.){
        printf("must satisfy von-neuman's stable conditions. actual ratio = %f", ratio);
        exit(1);
    }
    ...
    for (t = now_step; t < calc_step + now_step; t++)
    {
        ...
        for (i = start_yid; i < end_yid; i++)
        {
            for(k=0; k<domain_col.size(); k++){
                if (domain_col[k]["is_exp"].first == 0){
                    printf("only for explicit method.\n");
                    exit(1);
                }
                coef = domain_col[k]["coef"].first;

                start_xid = max((int)domain_col[k]["x"].first, 1);
                end_xid = domain_col[k]["x"].second;


                // cout << ratio_tx <<" " << ratio_ty << " coef:" << coef <<endl;
                
                ratio_x = ratio_tx * coef;
                ratio_y = ratio_ty * coef;
                if((ratio_x + ratio_y) >= 1./2.){
                    printf("must satisfy von-neuman's stable conditions. actual ratio = %f", ratio_x + ratio_y);
                    exit(1);
                }
                if(k>0){
                    coef_2 = domain_col[k-1]["coef"].first;
                    ratio_neighb = ratio_tx * 2 * (coef_2 * coef) / (coef_2 + coef);
                }else{
                    ratio_neighb = ratio_x;
                }

                j = start_xid;
                // printf("k=%d, j=%d, raito_n=%f, fvalue=%f\n", k, j, ratio_neighb, field[t - 1][i][j - 1]);

                x_diff = ratio_x * field[t - 1][i][j + 1] + ratio_neighb * field[t - 1][i][j - 1] - ratio_x * 2 * field[t - 1][i][j];
                y_diff = field[t - 1][i + 1][j] + field[t - 1][i - 1][j] - 2 * field[t - 1][i][j];
                field[t][i][j] = field[t - 1][i][j] + x_diff + ratio_y * y_diff;

                for (j = start_xid+1; j < end_xid-1; j++)
                {
                ...
                // cout << "j="<<j <<" max="<<end_xid-1<<endl;
                if(k<domain_col.size()-1){
                    coef_2 = domain_col[k+1]["coef"].first;
                    ratio_neighb = ratio_tx * 2 * (coef_2 * coef) / (coef_2 + coef);
                }else{
                    ratio_neighb = ratio_x;
                ...
                // cout << "k="<<k <<" max="<<domain_col.size()<<endl;
                // printf("k=%d, j=%d, raito_n=%f, fvalue=%f\n", k, j, ratio_neighb, field[t - 1][i][j + 1]);
                x_diff = ratio_neighb * field[t - 1][i][j + 1] + ratio_x * field[t - 1][i][j - 1] - ratio_x * 2 * field[t - 1][i][j];
                // cout << "ok" <<endl;
                y_diff = field[t - 1][i + 1][j] + field[t - 1][i - 1][j] - 2 * field[t - 1][i][j];
                // cout << "ok2" <<endl;
...
void sparse_implicit_solve_2d_xy_whole(int now_step, float calc_time_step, map<string, float> delta, vector<map<string, pair<float, float>>> domain_col)
{
    int t, i, j, k, index, x_idx, y_idx, coef_idx, field_xid, field_yid, coef_k, dense_index, max_gauss_coef_index, add_index, coef_row_base;
    int calc_step = int(calc_time_step / delta["t"]);
    int start_xid, end_xid, start_yid, end_yid, tmp_field_xid, tmp_field_yid;
    float x_diff, ratio_x, ratio_y, ratio_tx, ratio_ty, y_diff;
    map<string, pair<int, int>> col_status;
    start_xid = max((int)domain_col[0]["x"].first, 1);
    end_xid = domain_col[domain_col.size()-1]["x"].second;
    ...
    int f2c_idx = 0, small_edge, large_edge;
    int x_idx_num, y_idx_num, coef_n_rows, coef_n_cols;
    x_idx_num = end_xid - start_xid;
    y_idx_num = end_yid - start_yid;
    // if (y_idx_num < x_idx_num)
    ...
    float substitute_sum, coef1, coef2, large_edge_ratio, ratio;
    ...
    {
        x_idx = i % x_idx_num;
        small_edge = (x_idx+start_xid) == domain_col[k]["x"].second;
        large_edge = (x_idx+start_xid) == domain_col[k]["x"].second - 1;
        large_edge &= k < domain_col.size() - 1;
        // cout << "ii=" <<i <<" x="<<x_idx<<" k="<<k <<endl;
        if(small_edge || ((x_idx == 0) & (k > 0))){
            k = (k+1) % domain_col.size();
        }else if(large_edge){
        ...
        for (j = 0; j < index_list.size(); j++)
        {
            ...
            // cout << "ok" <<endl;
            if((j==1) & large_edge){
                ratio = - large_edge_ratio;
            }else if(j==0){
                ratio_x = coef1 * ratio_tx;
                ratio_y = coef1 * ratio_ty;
                ratio = 1 + 2 * ratio_x + 2 * ratio_y;
            }else if(j==1){
                ratio_x = coef1 * ratio_tx;
                ratio = - ratio_x;
            }else{
...
void solve_2d(map<string, pair<float, float>> spatio_bound, map<string, float> delta, vector<map<string, pair<float, float>>> domain_boundary)
{
    ...
    for (now_step = 1; now_step < n_rows; now_step++)
    {
        ...
        if(is_same){
            start = std::chrono::system_clock::now();
            if(is_exp){
                explicit_solve_2d_whole(now_step, delta_info["exp"]["t"], delta_info["exp"], domain_col);
            }else{
                sparse_implicit_solve_2d_xy_whole(now_step, delta_info["exp"]["t"], delta_info["imp"], domain_col);
            }
            end = std::chrono::system_clock::now();
            ...
            for (j = 0; j < domain_col.size(); j++)
            {
                ...
                if (domain_col[j]["is_exp"].first == 1)
                {
                    ...
                    explicit_solve_2d(now_step, delta_info["exp"]["t"], delta_info["exp"], domain_col[j], coef, neighb_coef);
                    end = std::chrono::system_clock::now();
                    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / 1000.0;
                    // cout << "finish exp" << endl;
                ...
                else
                {
                    ...
                    if (is_sparse_lu)
                    {
                        ...
                        if (col_status["x"].second - col_status["x"].first <= col_status["y"].second - col_status["y"].first){
                            // cout << "sparse xy now:" << now_step << endl;
                            // cout << domain_col[j] << endl;
                            sparse_symmetric_implicit_solve_2d_xy(now_step, delta_info["imp"]["t"], delta_info["imp"], domain_col[j], coef, neighb_coef);
                        }else{
                            // cout << "sparse yx" << endl;
                            // cout << domain_col[j] << endl;
...
int pde_2d(YAML::Node config, char *argv[], float type_value)
{
    ...
    if(try_time == 1){
        ...
        if (is_exact_solution)
            ...
            solve_2d(spatio_bound, delta, domain_list);
        ...
        for(i=0; i<try_time;i++){
            ...
            else solve_2d(spatio_bound, delta, domain_list);
            auto end = std::chrono::system_clock::now();
